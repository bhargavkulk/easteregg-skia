...
def main() -> None:
    ap = argparse.ArgumentParser(description='Read baseline values from report/index.html table and compute geomean.')
    ap.add_argument('index_html', nargs='?', default='report/index.html', type=Path)
    ap.add_argument('--print-baselines', action='store_true', help='Also print Python list: baselines = [...]')
    args = ap.parse_args()
    ...
    try:
        ...
        raise SystemExit('Could not find "Baseline" column in first table header')

    baselines: list[float] = []
    for row in rows[1:]:
        if baseline_idx >= len(row):
            continue
        v = parse_float(row[baseline_idx])
        if v is None or v <= 0:
            continue
        baselines.append(v)

    if not baselines:
        raise SystemExit('No baseline numeric values found')

    gm = geomean(baselines)

    print(f'index_html={index_html}')
    print(f'baseline_count={len(baselines)}')
    print(f'geomean_baseline_ms={gm:.9f}')

    if args.print_baselines:
        joined = ', '.join(f'{x:.9f}' for x in baselines)
        print(f'baselines = [{joined}]')
